...
### None is divided into three core parts with different JavaScript runtimes:
...
-   Run the `pnpm build`. (it runs `tsc && pnpm pack`).
...
-   `pnpm upshared` in frontend = `pnpm remove @none/shared && pnpm install && pnpm add ../shared/none-shared-2.3.0.tgz && pnpm install`
